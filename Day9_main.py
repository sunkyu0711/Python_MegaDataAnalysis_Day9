...
if user_pw=='1234': # 회원 가입 기능도 만들고 싶은데...
    ...
    select_data=['따릉이 분석','환율 분석(크롤링)']
    menu=st.sidebar.selectbox('메뉴를 선택하세요.',select_data,index=0) # index=0: 아무것도 선택되지 않게끔

    if menu=='따릉이 분석':
        st.write('따릉이 분석')
        data=Day9_bikes.DataPreprocessing()
        st.dataframe(data.head(20))
        Day9_bikes.Top50(data)
    else:
        st.write('환율 분석(크롤링)')

# 실행: 터미널 켜고 "stream run 9일차_main.py" 입력 후 엔터
